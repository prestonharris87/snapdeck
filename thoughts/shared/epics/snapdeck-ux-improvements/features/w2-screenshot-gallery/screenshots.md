...
The three popup-chrome states render the extension popup at its intrinsic
280px body width (height auto-grows to content). The re-open state is the
in-page editor overlay on the full host tab.
-->
...
## gallery-empty — Empty state (non-target / empty report)
...
## delete-confirm — Delete confirmation affordance
...
- **Proves:** AC "Click → lossless re-open (model-byte)" + AC "Overlay reuse … page-independent" + E2E "Click → re-open restores the model" (clicking a thumbnail re-opens that shot in the existing editor on the stored PNG with its model restored).
